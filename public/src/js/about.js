...
(async () => {
  ...
  if (currentVersion) {
    ...
    version.innerText = "Couldn't get latest version information";
  }

  // Messages are defined for different scenarios where the user is not on the latest released version.
  var oldMessage =
    "Warning: You are on a older version. The current version is %VERSION%";
  var betaMessage =
    "You are on a pre-release version! The current release is %VERSION%";
  var otherMessage =
    "You not on the currently released version. The current release is %VERSION%";
  ...
  if (latestVersion && currentVersion !== latestVersion) {
    ...
    var latestVersionNumber = latestVersion
      .replace("v", "")
      .replaceAll("-", ".");
    var firstStr = latestVersionNumber.search(/\./) + 1;
    latestVersionNumber = Number(
      latestVersionNumber.substr(0, firstStr) +
        latestVersionNumber.slice(firstStr).replace(/\./g, "")
    );

    var currentVersionNumber = currentVersion
      .replace("v", "")
      .replaceAll("-", ".");
    var firstStr2 = currentVersionNumber.search(/\./) + 1;
    currentVersionNumber = Number(
      currentVersionNumber.substr(0, firstStr2) +
        currentVersionNumber.slice(firstStr2).replace(/\./g, "")
    );
